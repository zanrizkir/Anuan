                  ...
                  @if (count($kota))
                    @foreach ($kota as $kotas)
                        <tr>
                            ...
                            <td>
                                <form action="{{ route('kota.destroy', $kotas->id) }}" method="post">
                                    @csrf
                                    @method('delete')
                                    <a href="{{ route('kota.edit', $kotas->id) }}"
                                        class="btn btn-sm btn-secondary" data-bs-toggle="tooltip"
                                        data-bs-offset="0,4" data-bs-placement="top" data-bs-html="true"
                                        title="Edit Data">
                                        edit
                                    </a> |
                                    <button type="button" class="btn btn-sm btn-danger" data-toggle="modal" data-target="#defaultModal{{ $kotas->id }}"> Hapus </button>
                                    <div class="modal fade" id="defaultModal{{ $kotas->id }}" tabindex="-1" role="dialog" aria-labelledby="defaultModalLabel" aria-hidden="true">
